import React from 'react'
import { ChevronDown, IndianRupee, LocateFixed, MapPin } from 'lucide-react'
const FinishRide = (props) => {
    return (
        <div>
            <h5 className='absolute top-0 left-1/2 -translate-x-1/2 p-2 cursor-pointer' onClick={() => {
                props.setFinishRidePanel(false)
            }}> <ChevronDown size={30} /></h5>
            <h3 className='text-2xl font-semibold mb-5'>Finish this Ride</h3>
            <div className='flex items-center justify-between p-4 rounded-lg  mt-4 bg-yellow-400'>
                <div className='flex items-center gap-3 '>
                    <img className='h-10 border-2 border-yellow-700 rounded-full object-cover w-10' src=" data:image/png;base64," alt="" />
                    <h2 className='text-lg font-medium capitalize'>Sudip Rakshit</h2>
                </div>
                <h5 className='text-lg font-semibold'>2.2 KM</h5>
            </div>
            <div className='flex gap-2 justify-between flex-col items-center'>
                <div className='w-full mt-5'>
                    <div className='flex items-center gap-5 p-3 border-b-2'>
                        <MapPin />
                        <div>
                            <h3 className='text-lg font-medium'>562/11-A</h3>
                            <p className='text-sm -mt-1 text-gray-600'>Bidhanager,ssb Srani</p>
                        </div>
                    </div>
                    <div className='flex items-center gap-5 p-3 border-b-2'>
                        <LocateFixed />
                        <div>
                            <h3 className='text-lg font-medium'>562/11-A</h3>
                            <p className='text-sm -mt-1 text-gray-600'>Benachity,Durgapur</p>
                        </div>
                    </div>
                    <div className='flex items-center gap-5 p-3'>
                        <IndianRupee />
                        <div>
                            <h3 className='text-lg font-medium'>223 </h3>
                            <p className='text-sm -mt-1 text-gray-600'>Cash Payment</p>
                        </div>
                    </div>
                </div>

                <div className='mt-10 w-full'>

                    <button
                        //onClick={endRide}
                        className='w-full mt-5 flex  text-lg justify-center bg-green-600 text-white font-semibold p-3 rounded-lg'>
                        Finish Ride
                    </button>


                </div>
            </div>
        </div>
    )
}

export default FinishRide